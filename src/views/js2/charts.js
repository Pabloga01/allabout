...
(async () => {
	...
	const query3 = await fetch('http://localhost:3000/backend/api/allpublicationsbycountry',);
	const data3 = await query3.json();
	let arCountries2 = [];
	let arCountByCountry2 = [];
	data3.forEach(element => {
		arCountByCountry2.push(element.count);
		arCountries2.push(element.country);
	});
	if (typeof data !== 'false') {
		const ctx = document.querySelector("#pieChartCountry").getContext('2d');
		const myChart = new Chart(ctx, {
			type: 'pie',
			data: {
				labels: arCountries2,
